...
from neo4j.v1 import TRUST_SIGNED_CERTIFICATES, SSL_AVAILABLE, CypherError
...
from neo4j.v1 import GraphDatabase as _GraphDatabase
# end::minimal-example-import[]
# (* "good reason" is defined as knowing what you are doing)


class GraphDatabase(_GraphDatabase):

    @classmethod
    def driver(cls, uri, **config):
        auth = (IntegrationTestCase.user, IntegrationTestCase.password)
        return super(GraphDatabase, cls).driver(IntegrationTestCase.bolt_uri, auth=auth)
...
class MinimalWorkingExampleTestCase(IntegrationTestCase):
...
class ExamplesTestCase(IntegrationTestCase):
    ...
    def test_configuration(self):
        # tag::configuration[]
        driver = GraphDatabase.driver("bolt://localhost:7687", auth=("user", "password"), max_pool_size=10)
        # end::configuration[]
        return driver
    ...
    def test_tls_require_encryption(self):
        ...
        driver = GraphDatabase.driver("bolt://localhost:7687", auth=("user", "password"), encrypted=True)
        # end::tls-require-encryption[]

    @skip("testing verified certificates not yet supported ")
    def test_tls_signed(self):
        # tag::tls-signed[]
        driver = GraphDatabase.driver("bolt://localhost:7687", auth=("user", "password"), encrypted=True, trust=TRUST_SIGNED_CERTIFICATES)
        # end::tls-signed[]
        assert driver
    ...
    def test_transaction_commit(self):
        driver = GraphDatabase.driver("bolt://localhost:7687", auth=("user", "password"))
        session = driver.session()
        session.run("MATCH (p:Person {name: 'Guinevere'}) DELETE p")
