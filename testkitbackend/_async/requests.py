...
async def NewAuthTokenManager(backend, data):
    ...
    await backend.send_response("AuthTokenManager",
...
async def NewTemporalAuthTokenManager(backend, data):
    auth_token_manager_id = backend.next_key()

    async def auth_token_provider():
        key = backend.next_key()
        await backend.send_response("TemporalAuthTokenProviderRequest", {
            "id": key,
    ...
    auth_manager = AsyncAuthManagers.temporal(auth_token_provider)
    backend.auth_token_managers[auth_token_manager_id] = auth_manager
    await backend.send_response("TemporalAuthTokenManager",
                                {"id": auth_token_manager_id})


async def TemporalAuthTokenProviderCompleted(backend, data):
