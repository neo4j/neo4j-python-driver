...
def pool():
    ...
    pool.acquire = Mock(side_effect=acquire_side_effect)
    return pool
...
@mark_sync_test
def test_session_run_wrong_values(pool, query, error_type):
    with Session(pool, SessionConfig()) as session:
        with pytest.raises(error_type):
            session.run(query)
...
@mark_sync_test
def test_session_run_query_argument(pool, as_string, mocker):
    query_str = "foobar"
