...
@mark_sync_test
def test_acquire_performs_no_lifeness_check_on_fresh_connection(
    opener, lifeness_timeout
):
    pool = Neo4jPool(
        opener, PoolConfig(), WorkspaceConfig(), ROUTER_ADDRESS
    )
    cx1 = pool._acquire(READER_ADDRESS, 30, lifeness_timeout)
    assert cx1.addr == READER_ADDRESS
    cx1.reset.asset_not_called()
...
@pytest.mark.parametrize("lifeness_timeout", (0, 1, 2))
@mark_sync_test
def test_acquire_performs_lifeness_check_on_existing_connection(
    opener, lifeness_timeout
):
    pool = Neo4jPool(
        opener, PoolConfig(), WorkspaceConfig(), ROUTER_ADDRESS
    ...
    cx2 = pool._acquire(READER_ADDRESS, 30, lifeness_timeout)
    assert cx1 is cx2
...
@mark_sync_test
def test_acquire_creates_connection_on_failed_lifeness_check(
...
@pytest.mark.parametrize("lifeness_error",
                         (OSError, ServiceUnavailable, SessionExpired))
@mark_sync_test
def test_acquire_returns_other_connection_on_failed_lifeness_check(
    opener, lifeness_error
):
    def lifeness_side_effect(*args, **kwargs):
        raise lifeness_error("lifeness check failed")
    ...
    # then acquire it again and assert the lifeness check was performed
    cx3 = pool._acquire(READER_ADDRESS, 30, lifeness_timeout)
    assert cx3 is cx2
    cx1.is_idle_for.assert_called_once_with(lifeness_timeout)
    cx1.reset.assert_called_once()
