...
@mark_sync_test
def test_return_initial_bookmarks_for_the_given_db(db) -> None:
...
@mark_sync_test
def test_return_get_bookmarks_from_bookmark_supplier(
...
@mark_sync_test
def test_chains_bookmarks_for_existing_db() -> None:
    initial_bookmarks: t.Dict[str, t.List[str]] = {
        "db1": ["db1:bm1", "db1:bm1"],
        "db2": [],
        "db3": ["db3:bm1", "db3:bm2"],
...
@mark_sync_test
def test_add_bookmarks_for_a_non_existing_database() -> None:
    initial_bookmarks: t.Dict[str, t.List[str]] = {
        "db1": ["db1:bm1", "db1:bm1"],
        "db2": [],
        "db3": ["db3:bm1", "db3:bm2"],
    ...
    new_bookmarks = bmm.get_bookmarks("db5")
    all_bookmarks = bmm.get_all_bookmarks()
...
def test_notify_on_new_bookmarks(
    ...
    bmm.update_bookmarks(db, bookmarks_old, bookmarks_new)
...
def test_does_not_notify_on_empty_new_bookmark_set(
    ...
    bmm.update_bookmarks(db, ["db1:bm1"], [])
...
@mark_sync_test
def test_forget_database(dbs) -> None:
    initial_bookmarks: t.Dict[str, t.List[str]] = {
        "db1": ["db1:bm1", "db1:bm1", "db1:bm2"],
        "db2": ["db2:bm1"],
    }
    ...
    assert isinstance(bmm, Neo4jBookmarkManager)
    assert (set(bmm._bookmarks.keys())
            == set(initial_bookmarks.keys()) - set(dbs))

    for db in dbs:
        assert bmm.get_bookmarks(db) == set()
    assert bmm.get_all_bookmarks() == set(
